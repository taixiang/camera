...
public class RainView extends View {
    ...
    private void initData() {
        for (int i = 0; i < 20; i++) {
            ...
            itemImg.bitmap = BitmapFactory.decodeResource(getResources(), imgResId);
            //起始横坐标在[100,getWidth()-100) 之间
            itemImg.x = random.nextInt(getWidth() - 200) + 100;
            //起始纵坐标在(-getHeight(),0] 之间，即一开始位于屏幕上方以外
            itemImg.y = -random.nextInt(getHeight());
            //横向偏移[-2,2) ，即左右摇摆区间
            itemImg.offsetX = random.nextInt(4) - 2;
            //纵向固定下落12
            itemImg.offsetY = 12;
            //缩放比例[0.8,1.2) 之间
            itemImg.scale = (float) (random.nextInt(40) + 80) / 100f;
